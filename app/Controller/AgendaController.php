...
class AgendaController extends AppController {
	...
	private function getMediosLst() {
		$medios_lst = $this->MedioPublicidad->find( 'all', array(
				'callbacks' => false,
				'order'     => array( 'MedioPublicidad.id' )
		) );
	...
	function listado() {
		...
		$this->set( 'tiposEvento', array( '' => '-- todas las acciones --' ) + array(
						'Acciones con clientes' => $this->getAccionesLst(),
						'Tareas'                => $this->getTareasLst()
				) );
			...
			switch ( $this->passedArgs[0] ) {
				case 'inmueble_id':
					...
					$info = $this->Inmueble->find( 'first', array(
							'conditions' => array( 'Inmueble.id' => $this->passedArgs[1] ),
							'recursive'  => 0,
				...
				case 'propietario_id':
					...
					$info = $this->Propietario->find( 'first', array(
							'conditions' => array( 'Propietario.id' => $this->passedArgs[1] ),
							'recursive'  => 0,
				...
				case 'demandante_id':
					...
					$info = $this->Demandante->find( 'first', array(
							'conditions' => array( 'Demandante.id' => $this->passedArgs[1] ),
							'recursive'  => 0,
	...
	public function edit( $id = null ) {
		...
		if ( $id != null ) {
			...
			$info                = $this->Evento->find( 'first', array(
					'conditions' => array(
							'Evento.id'         => $id,
